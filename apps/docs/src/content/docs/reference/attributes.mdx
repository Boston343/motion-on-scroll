...
<Aside>
  This is included for parity with AOS, but MOS also supports `data-mos-amount` for more fine-grained control.
</Aside>
...
Numeric override (0–1) passed directly to `inView` and takes precedence over `data-mos-anchor-placement`. Useful for fine-grained control. `0` represents when "some" of an element enters the viewport, and `1` represents when the entire element is inside the viewport.
