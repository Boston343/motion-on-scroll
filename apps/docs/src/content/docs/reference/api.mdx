...
Initialises observers for every element possessing `data-mos`. Call this once on page load. It’s idempotent—calling it again merges new defaults and refreshes observation.
...
<Aside>
  <strong>Hybrid engine.</strong> Motion automatically chooses the most performant path:
  it runs animations with the browser-native <em>Web Animations API</em> whenever the animated value is supported, and falls back to JavaScript (`requestAnimationFrame`) when it isn’t. Either way, MOS abstracts this detail—you use the same API.
</Aside>

## refreshHard()

Re-evaluates the DOM for all `data-mos` elements and re-attaches observers using the last supplied global options. Useful after client-side navigation or significant DOM changes. This is internally called automatically when DOM changes are detected, unless you disable the mutation observer.
...
```ts
import { init, refreshHard, registerAnimation, registerEasing, registerKeyframes } from 'motion-on-scroll';
```
